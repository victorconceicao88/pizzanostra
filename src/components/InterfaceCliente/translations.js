// translations.js
export const translations = {
  pt: {
    // Header
    restaurantName: "Pizza Nostra Restaurante & Pizzaria",
    cart: "Carrinho",
    

    // Hero section
    since: "Desde 1985",
    title: "Autêntica Pizza Brasileira em Portimão",
    subtitle: "Feita com ingredientes importados da Itália e forno a lenha tradicional. Entregamos em toda a região!",
    viewCart: "Ver Carrinho",
    stampsAvailable:"Selos disponíveis",
    useStamps:"Utilizar selos",
    usingStamps:"Selos aplicados",
    fullMenu: "Cardápio Completo",
    halfAndHalf: "Montar com dois sabores (meia a meia)?",
    firstHalf: "Primeira metade",
          secondHalf: "Segunda metade",
          selectHalf: "Selecione o sabor",
          halfPizzaNotice: "Escolha dois sabores para sua pizza (apenas para tamanho família)",
          chooseMeatType:"Qual carne você gostaria?",
          selectMeatTypeRequired:"Você precisa escolher a carne antes de seguir.",
          meatType:"Tipo de Carne",
          includeMenu:"Deseja incluir o menu?",
          menuIncludes: "Este menu vem com:",
          withMenu:"Com menu",
          withoutMenu:"Versão simples (sem menu)",
          Size: "Tamanho",
          Stuffed: "Borda Recheada",
          extras: "Extras",
          freeDrink: "Bebida Grátis",
          review: "Revisão",
          halfAndHalf: "Meia a Meia",
          halfAndHalfDescription: "Escolha dois sabores em uma pizza família",
          chooseSize: "Escolha o Tamanho",
          individual: "Individual",
          media: "Média",
          familia: "Família",
          individualDesc: "1 pessoa",
          mediaDesc: "1-2 pessoas",
          familiaDesc: "3-4 pessoas",
          nextStep: "Próximo Passo",
          chooseBorder: "Escolha a Borda",
          noBorder: "Sem Borda",
          stuffedBorderDesc: "Deliciosa borda recheada",
          back: "Voltar",
          chooseExtras: "Escolha os Extras",
          chooseFreeDrink: "Escolha sua Bebida Grátis",
          reviewOrder: "Revisar Pedido",
          reviewYourOrder: "Revise seu Pedido",
          customizedItem: "Item personalizado",
          size: "Tamanho",
          doughType: "Tipo de Massa",
          thinDough: "Fina",
          thinDoughDesc: "Massa fina e crocante",
          thickDough: "Grossa",
          thickDoughDesc: "Massa grossa e macia",
          firstHalf: "Primeira Metade",
          secondHalf: "Segunda Metade",
          selectHalf: "Selecione um sabor",
          border: "Borda",
          quantity: "Quantidade",
          total: "Total",
          addToCart: "Adicionar ao Carrinho",
          selectSecondHalf: "Por favor, selecione o segundo sabor",
          customizeYourOrder: "Personalize seu pedido",
          notSelected: "Não selecionado",

    
                 
          // Bordas
          borderType: "Tipo de Borda",
          thinBorder: "Fina (tradicional)",
          thickBorder: "Grossa (recheada)",
          stuffedCrust: "Borda Recheada",
          noBorder: "Sem borda recheada",
          cheeseCrust: "Borda de Queijo",
          pepperoniCrust: "Borda de Pepperoni",
          catupiryCrust: "Borda de Catupiry",
          
          // Massas
          pasta: "Massas",
          pastaOptions: "Opções de Massa",
          penne: "Penne",
          spaghetti: "Espaguete",
          fettuccine: "Fettuccine",
          gnocchi: "Nhoque",
          choosePasta: "Escolha seu tipo de massa",
          pastaWith: "Massa com",
          
          // Tamanhos
          individual: "Individual (25cm)",
          media: "Média (32cm)",
          familia: "Família (41cm)",
          size: "Tamanho",
          
          // Outros
          extras: "Extras",
          addToCart: "Adicionar ao Carrinho",
          Quantity: "Quantidade",
          total: "Total",
          extrasTotal: "Total de extras",
    
    // Categories

    entradas: "Entradas",
    petiscos: "Petiscos",
    tradicionais: "Tradicionais",
    vegetarianas: "Vegetarianas",
    doces: "Doces",
    massas: "Massas",
    todos: "Todos os Dias",
    sobremesas: "Sobremesas",
    bebidas: "Bebidas",
    vinhos: "Vinhos",
    
    // Product actions
    continueToPayment:'Ir Para Pagamento >>',
    stampsEarned:"Selos Ganhos",
    add : "Adicionar",
    addToCart: "Adicionar",
    popular: "Popular",
    new: "Novo",
    useStampsForItem:"Pagar Com Selos",
    paidWithStamps : "Pago Com Selos",
    stampsUsed : "Selos Utilizados",
    Size : "Escolher Tamanho",
    Stuffed: "Selecionar Borda",
  
    
    // Cart
    close:'Fechar',
    orderSentSuccess:'Pedido enviado com sucesso',
    yourCart: "Seu Carrinho",
    items: "itens",
    emptyCart: "Seu carrinho está vazio",
    emptyCartMessage: "Adicione deliciosas pizzas para começar!",
    exploreMenu: "Explorar Cardápio",
    subtotal: "Subtotal",
    deliveryFee: "Taxa de entrega",
    free: "Grátis",
    stamps: "Selos disponíveis",
    estimatedTotal: "Total estimado",
    proceedToCheckout: "Prosseguir para Checkout",
  
    
    // Checkout steps
    deliveryInfo: "Informações de Entrega",
    paymentMethod: "Método de Pagamento",
    orderConfirmed: "Pedido Confirmado!",
    
    // Delivery options
    pickup: "Retirada no Local",
    pickupAddress: "Rua da Pizza, 123 - Portimão",
    delivery: "Entrega em Casa",
    deliveryArea: "Entregamos na sua região",
    openingHours: "Horario de Funcionamento",
    quantity: "Quantidade",
    
    // Form fields
    fullName: "Nome Completo *",
    phone: "Telefone *",
    address: "Endereço Completo *",
    addressPlaceholder: "Rua, número, apartamento",
    neighborhood: "Localidade *",
    selectNeighborhood: "Selecione sua localidade",
    deliveryZones: "Ver zonas de entrega",
    notes: "Observações",
    notesPlaceholder: "Alguma instrução especial para a entrega?",

    
    // Payment methods
    mbway: "MB Way",
    mbwayDescription: "Pagamento instantâneo",
    cash: "Dinheiro",
    cashDescription: "Troco necessário",
    card: "Cartão",
    cardDescription: "Na entrega",
    mbwayNumber: "Número de Telemóvel MB Way *",
    changeFor: "Precisa de troco para quanto? *",
    changeExample: "Ex: 20.00",
    size:"Tamanho",
    
    // Order summary
    orderSummary: "Resumo do Pedido",
    discount: "Desconto",
    total: "Total",
    estimatedTime: "Tempo estimado",
    
    // Confirmation
    confirmationMessagePickup: "Sua pizza estará pronta para retirada em aproximadamente 30 minutos.",
    confirmationMessageDelivery: "Nosso motoboy saiu para entrega e chegará em aproximadamente 45 minutos.",
    orderNumber: "Número do Pedido:",
    paymentMethodLabel: "Forma de Pagamento:",
    whatsappMessage: "Acompanhe seu pedido pelo WhatsApp",
    newOrder: "Fazer Novo Pedido",
    
    // Product descriptions (exemplos)
    extrasTotal:'Total de Extras',
    quantity:'Quantidade',
    addExtras:'Adicionar Extras',
    noBorder:'Sem Borda',
    border: 'Borda',
    StuffedCrust: 'Selecione a Borda',
    margheritaDesc: "Molho de tomate, mussarela, manjericão fresco",
    pepperoniDesc: "Molho de tomate, mussarela e pepperoni",
    onlyChargeDelivery:"Só pagas a entrega.",
    registerToEarn:"Cadastre-se para ganhar",
    registerToEarnDescription:"Cadastre-se gratuitamente e comece a ganhar recompensas." ,
    registerNow:"Cadastre-se agora",
    orderPreparingTitle: "O seu pedido está a ser preparado com todo o cuidado!",
    orderPreparingDescription: "Se precisar de alguma informação, fale connosco através do:",
  },
  
  en: {
    // Header
    restaurantName: "Pizza Nostra Restaurant & Pizzeria",
    cart: "Cart",
    
    // Hero section
    since: "Since 1985",
    title: "Authentic Brazilian Pizza in Portimão",
    subtitle: "Made with ingredients imported from Italy and traditional wood-fired oven. We deliver throughout the region!",
    viewCart: "View Cart",
    fullMenu: "Full Menu",
    useStampsForItem:"Redeem stamps for item",
    stampsEarned:"You're collecting stamps.",
    orderSentSuccess:"Thanks! We’ve received your order and we’re getting it ready",
    orderPreparingTitle: "We're preparing your order with care!",
    orderPreparingDescription: "If you need anything, feel free to give us a call:",
    stampsAvailable:"Stamps Balance",
    useStamps:"Redeem Stamps",
    usingStamps:"Stamps Applied",
    halfAndHalf:"Would you like to make it Half and Half?",
    thinBorder: "Thin Crust (Traditional)",
    thickBorder: "Thick Crust (Stuffed)",
    borderType:"Crust Type",
    Size: "Size",
  Stuffed: "Stuffed Crust",
  extras: "Extras",
  freeDrink: "Free Drink",
  review: "Review",
  halfAndHalf: "Half & Half",
  halfAndHalfDescription: "Choose two flavors in one family pizza",
  chooseSize: "Choose Size",
  individual: "Individual",
  media: "Medium",
  familia: "Family",
  individualDesc: "1 person",
  mediaDesc: "1-2 people",
  familiaDesc: "3-4 people",
  nextStep: "Next Step",
  chooseBorder: "Choose Crust",
  noBorder: "No Crust",
  stuffedBorderDesc: "Delicious stuffed crust",
  back: "Back",
  chooseExtras: "Choose Extras",
  chooseFreeDrink: "Choose Your Free Drink",
  reviewOrder: "Review Order",
  reviewYourOrder: "Review Your Order",
  customizedItem: "Customized item",
  size: "Size",
  doughType: "Dough Type",
  thinDough: "Thin",
  thinDoughDesc: "Thin and crispy crust",
  thickDough: "Thick",
  thickDoughDesc: "Thick and soft crust",
  firstHalf: "First Half",
  secondHalf: "Second Half",
  selectHalf: "Select a flavor",
  border: "Crust",
  quantity: "Quantity",
  total: "Total",
  addToCart: "Add to Cart",
  selectSecondHalf: "Please select the second flavor",
  customizeYourOrder: "Customize your order",
  notSelected: "Not selected",

    
    // Categories
    entradas: "Starters",
    petiscos: "Snacks",
    tradicionais: "Traditional",
    vegetarianas: "Vegetarian",
    doces: "Sweet",
    massas: "Pasta",
    todos: "Daily Specials",
    sobremesas: "Desserts",
    bebidas: "Drinks",
    vinhos: "Wines",
    
    // Product actions

   
    addToCart: "Add to Cart",
    popular: "Popular",
    new: "New",
    
    // Cart
    yourCart: "Your Cart",
    items: "items",
    emptyCart: "Your cart is empty",
    emptyCartMessage: "Add delicious pizzas to get started!",
    exploreMenu: "Explore Menu",
    subtotal: "Subtotal",
    deliveryFee: "Delivery fee",
    free: "Free",
    stamps: "Stamps available",
    estimatedTotal: "Estimated total",
    proceedToCheckout: "Proceed to Checkout",
    firstHalf:"First half selected",
    secondHalf :"Now select the second half:",
    selectHalf :"----------",
    
    // Checkout steps
    deliveryInfo: "Delivery Information",
    paymentMethod: "Payment Method",
    orderConfirmed: "Order Confirmed!",
    
    // Delivery options
    pickup: "Pickup",
    pickupAddress: "Pizza Street, 123 - Portimão",
    delivery: "Home Delivery",
    deliveryArea: "We deliver in your area",
    paidWithStamps : "Paid with Stamps",

    
    // Form fields
    registerToEarn:"Sign up to earn",
    registerToEarnDescription:"Sign up for free and start earning rewards." ,
    registerNow:"Sign up now",
    fullName: "Full Name *",
    phone: "Phone *",
    address: "Full Address *",
    addressPlaceholder: "Street, number, apartment",
    neighborhood: "Neighborhood *",
    selectNeighborhood: "Select your neighborhood",
    deliveryZones: "View delivery zones",
    notes: "Notes",
    notesPlaceholder: "Any special delivery instructions?",
    
    // Payment methods
    mbway: "MB Way",
    mbwayDescription: "Instant payment",
    cash: "Cash",
    cashDescription: "Change needed",
    card: "Card",
    cardDescription: "On delivery",
    mbwayNumber: "MB Way Phone Number *",
    changeFor: "Change for how much? *",
    changeExample: "Ex: 20.00",
    
    // Order summary
    orderSummary: "Order Summary",
    discount: "Discount",
    total: "Total",
    estimatedTime: "Estimated time",
    
    // Confirmation
    confirmationMessagePickup: "Your pizza will be ready for pickup in approximately 30 minutes.",
    confirmationMessageDelivery: "Our delivery person is on the way and will arrive in approximately 45 minutes.",
    orderNumber: "Order Number:",
    paymentMethodLabel: "Payment Method:",
    whatsappMessage: "Track your order via WhatsApp",
    newOrder: "Place New Order",
    
    // Product descriptions
    margheritaDesc: "Tomato sauce, mozzarella, fresh basil",
    pepperoniDesc: "Tomato sauce, mozzarella and pepperoni",
    chooseMeatType:"Choose your meat",
    selectMeatTypeRequired:"Please select a meat to continue",
    meatType:"Meat",
    includeMenu:"Would you like to upgrade to a combo?",
    menuIncludes:"Combo includes:",
    withMenu:"Meal Deal",
    withoutMenu:"No Meal"
  },
  
  es: {
    // Header
    restaurantName: "Pizza Nostra Restaurante & Pizzería",
    cart: "Carrito",
    
    // Hero section
    since: "Desde 1985",
    title: "Auténtica Pizza Brasileña en Portimão",
    subtitle: "Hecha con ingredientes importados de Italia y horno de leña tradicional. ¡Entregamos en toda la región!",
    viewCart: "Ver Carrito",
    stampsAvailable: "Sellos disponibles",
    useStamps: "Usar sellos",
    usingStamps: "Sellos aplicados",
    fullMenu: "Menú Completo",
    halfAndHalf: "Mitad y Mitad",
    firstHalf: "Primera mitad",
    secondHalf: "Segunda mitad",
    selectHalf: "Seleccione el sabor",
    halfPizzaNotice: "Elija dos sabores para su pizza (solo para tamaño familiar)",
    chooseMeatType: "¿Qué tipo de carne desea?",
    selectMeatTypeRequired: "Debe elegir la carne para continuar",
    meatType: "Tipo de Carne",
    includeMenu: "¿Desea incluir el menú?",
    menuIncludes: "Este menú incluye:",
    withMenu: "Con menú",
    withoutMenu: "Versión simple (sin menú)",
     Size: "Tamaño",
  Stuffed: "Borde Relleno",
  extras: "Extras",
  freeDrink: "Bebida Gratis",
  review: "Revisar",
  halfAndHalf: "Mitad y Mitad",
  halfAndHalfDescription: "Elige dos sabores en una pizza familiar",
  chooseSize: "Elige el Tamaño",
  individual: "Individual",
  media: "Mediana",
  familia: "Familiar",
  individualDesc: "1 persona",
  mediaDesc: "1-2 personas",
  familiaDesc: "3-4 personas",
  nextStep: "Siguiente Paso",
  chooseBorder: "Elige el Borde",
  noBorder: "Sin Borde",
  stuffedBorderDesc: "Delicioso borde relleno",
  back: "Atrás",
  chooseExtras: "Elige Extras",
  chooseFreeDrink: "Elige tu Bebida Gratis",
  reviewOrder: "Revisar Pedido",
  reviewYourOrder: "Revisa tu Pedido",
  customizedItem: "Artículo personalizado",
  size: "Tamaño",
  doughType: "Tipo de Masa",
  thinDough: "Fina",
  thinDoughDesc: "Masa fina y crujiente",
  thickDough: "Gruesa",
  thickDoughDesc: "Masa gruesa y suave",
  firstHalf: "Primera Mitad",
  secondHalf: "Segunda Mitad",
  selectHalf: "Selecciona un sabor",
  border: "Borde",
  quantity: "Cantidad",
  total: "Total",
  addToCart: "Añadir al Carrito",
  selectSecondHalf: "Por favor selecciona el segundo sabor",
  customizeYourOrder: "Personaliza tu pedido",
  notSelected: "No seleccionado",

    // Bordes
    borderType: "Tipo de Borde",
    thinBorder: "Fina (tradicional)",
    thickBorder: "Gruesa (rellena)",
    stuffedCrust: "Borde Relleno",
    noBorder: "Sin borde relleno",
    cheeseCrust: "Borde de Queso",
    pepperoniCrust: "Borde de Pepperoni",
    catupiryCrust: "Borde de Catupiry",
    
    // Pastas
    pasta: "Pastas",
    pastaOptions: "Opciones de Pasta",
    penne: "Penne",
    spaghetti: "Espagueti",
    fettuccine: "Fettuccine",
    gnocchi: "Ñoquis",
    choosePasta: "Elija su tipo de pasta",
    pastaWith: "Pasta con",
    
    // Tamaños
    individual: "Individual (25cm)",
    media: "Mediana (32cm)",
    familia: "Familiar (41cm)",
    size: "Tamaño",
    
    // Otros
    extras: "Extras",
    addToCart: "Añadir al Carrito",
    Quantity: "Cantidad",
    total: "Total",
    extrasTotal: "Total de extras",
    
    // Categorías
    entradas: "Entrantes",
    petiscos: "Aperitivos",
    tradicionais: "Tradicionales",
    vegetarianas: "Vegetarianas",
    doces: "Dulces",
    massas: "Pastas",
    todos: "Todos los Días",
    sobremesas: "Postres",
    bebidas: "Bebidas",
    vinhos: "Vinos",
    
    // Acciones de producto
    continueToPayment: "Ir al Pago >>",
    stampsEarned: "Sellos Ganados",
    add: "Añadir",
    addToCart: "Añadir",
    popular: "Popular",
    new: "Nuevo",
    useStampsForItem: "Pagar con Sellos",
    paidWithStamps: "Pagado con Sellos",
    stampsUsed: "Sellos Utilizados",
    Size: "Elegir Tamaño",
    Stuffed: "Seleccionar Borde",
  
    // Carrito
    close: "Cerrar",
    orderSentSuccess: "Pedido enviado con éxito",
    yourCart: "Tu Carrito",
    items: "artículos",
    emptyCart: "Tu carrito está vacío",
    emptyCartMessage: "¡Añade deliciosas pizzas para empezar!",
    exploreMenu: "Explorar Menú",
    subtotal: "Subtotal",
    deliveryFee: "Gastos de envío",
    free: "Gratis",
    stamps: "Sellos disponibles",
    estimatedTotal: "Total estimado",
    proceedToCheckout: "Proceder al Pago",
  
    // Pasos del pago
    deliveryInfo: "Información de Entrega",
    paymentMethod: "Método de Pago",
    orderConfirmed: "¡Pedido Confirmado!",
    
    // Opciones de entrega
    pickup: "Recogida en Local",
    pickupAddress: "Calle de la Pizza, 123 - Portimão",
    delivery: "Entrega a Domicilio",
    deliveryArea: "Entregamos en tu zona",
    openingHours: "Horario de Apertura",
    quantity: "Cantidad",
    
    // Campos del formulario
    fullName: "Nombre Completo *",
    phone: "Teléfono *",
    address: "Dirección Completa *",
    addressPlaceholder: "Calle, número, apartamento",
    neighborhood: "Localidad *",
    selectNeighborhood: "Seleccione su localidad",
    deliveryZones: "Ver zonas de entrega",
    notes: "Observaciones",
    notesPlaceholder: "¿Alguna instrucción especial para la entrega?",
    
    // Métodos de pago
    mbway: "MB Way",
    mbwayDescription: "Pago instantáneo",
    cash: "Efectivo",
    cashDescription: "Cambio necesario",
    card: "Tarjeta",
    cardDescription: "En la entrega",
    mbwayNumber: "Número de Teléfono MB Way *",
    changeFor: "¿Cambio para cuánto? *",
    changeExample: "Ej: 20.00",
    size: "Tamaño",
    
    // Resumen del pedido
    orderSummary: "Resumen del Pedido",
    discount: "Descuento",
    total: "Total",
    estimatedTime: "Tiempo estimado",
    
    // Confirmación
    confirmationMessagePickup: "Tu pizza estará lista para recoger en aproximadamente 30 minutos.",
    confirmationMessageDelivery: "Nuestro repartidor salió para la entrega y llegará en aproximadamente 45 minutos.",
    orderNumber: "Número de Pedido:",
    paymentMethodLabel: "Método de Pago:",
    whatsappMessage: "Sigue tu pedido por WhatsApp",
    newOrder: "Hacer Nuevo Pedido",
    
    // Descripciones de producto
    extrasTotal: "Total de Extras",
    quantity: "Cantidad",
    addExtras: "Añadir Extras",
    noBorder: "Sin Borde",
    border: "Borde",
    StuffedCrust: "Seleccionar Borde",
    margheritaDesc: "Salsa de tomate, mozzarella, albahaca fresca",
    pepperoniDesc: "Salsa de tomate, mozzarella y pepperoni",
    onlyChargeDelivery: "Solo pagas el envío.",
    registerToEarn: "Regístrate para ganar",
    registerToEarnDescription: "Regístrate gratuitamente y comienza a ganar recompensas.",
    registerNow: "Regístrate ahora",
    orderPreparingTitle: "¡Tu pedido está siendo preparado con todo el cuidado!",
    orderPreparingDescription: "Si necesitas alguna información, contáctanos a través de:",
  }
};


// Configuração de zonas de entrega com tradução
export const deliveryAreas = {
  "Zona 1 - Centro": {
    bairros: {
      pt: ["Portimão (Freguesia)", "Praia da Rocha", "Alto do Quintão"],
      en: ["Portimão (Parish)", "Praia da Rocha", "Alto do Quintão"],
      es: ["Portimão (Parroquia)", "Praia da Rocha", "Alto do Quintão"]
    },
    taxa: 1.00,
    tempo: {
      pt: "15-25 min",
      en: "15-25 min",
      es: "15-25 min"
    },
    color: "bg-emerald-100 border-emerald-500 text-emerald-800"
  },
  // ... outras zonas
};

// Função para obter tradução
export const t = (lang, key) => {
  return translations[lang][key] || key;
};